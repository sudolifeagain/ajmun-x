...
* **目的:** 300人規模・4日間開催の大会において、Discordアカウントと連携したQRコードによる高速かつスムーズな出席管理を行う。  
* **コアコンセプト:** 「行列を作らない」「最小権限でセキュアに」「サーバー/ロールごとの視覚的判別」。
* **サーバー構成:** 
  * **会議サーバー（6つ）:** 各会議ごとに1サーバー。それぞれ「参加者」「会議フロント」「スタッフ」が所属。
  * **運営サーバー（1つ）:** 全会議の「会議フロント」と「スタッフ」が所属する中央管理用サーバー。
  * 各サーバーにBotは参加可能であり、どのサーバーからでも参加者情報を取得可能とする。
* **ユーザー属性（3種類）:**
  * **参加者:** 各会議の一般参加者。会議サーバーのみに所属。
  * **会議フロント:** 特定会議の運営担当。**運営サーバーのロール**で判別。
...
* **イベント期間:**
  * **開催期間:** 2025年12月27日〜12月30日（日本時間）
  * **QRコード発行開始:** 2025年12月23日（日本時間）から事前発行可能
...
3. **属性決定**
   * ロール所持状況に基づき、ユーザーの属性（参加者/会議フロント/スタッフ）を決定してDBにキャッシュする。
...
2.  **視覚・聴覚フィードバック (UX)**
    ...
    *   **⚠️ バイブレーション機能について:**
        *   `navigator.vibrate()` APIはAndroid端末のChrome/Firefox等でのみ動作する。
        *   **iOS端末では全ブラウザ（Safari, Chrome, Firefox等）で非対応**（AppleがiOS上の全ブラウザにWebKitエンジンの使用を義務付けており、WebKitがVibration APIを実装していないため）。
        *   実装は行ったが、iOS非対応のため主要フィードバック手段としては採用せず、視覚フィードバック（背景色・アイコン変更）を主とする。
...
1.  **/attendance status [会議名|all] [属性?]**
    ...
    *   オプションで属性（参加者/会議フロント/スタッフ）を指定して絞り込み可能。
...
| primary\_attribute | Enum | 参加者/会議フロント/スタッフ（優先順位で決定） |
...
| attribute | Enum | 記録時の属性（参加者/会議フロント/スタッフ） |
| check\_in\_date | Date | 入場日 (ユニーク制約: user\_id + date) |

## **6\. 仕様変更履歴・追記 (Change Log)**

### **6-1. 入場フローの変更 (2025/12/25)**
*   **変更前:** 参加者全員がWebアプリケーションにアクセスし、「Discordでログイン」を行ってQRコードを表示する。
*   **変更後:** **Discord Botから全参加者にQRコードをDMで一斉送信する**方式を基本とする。WebログインはDM不達時のバックアップ手段として残す。
*   **理由:** 
    *   スマホでDiscordアカウントにログインする際、メールアドレスやパスワードを忘れているユーザーが多数発生し、ログインできずに入場できないトラブルが頻発することが予想されたため。
    *   こちらからプッシュ型でQRコード（有効な署名付きトークン）を配布することで、当日会場でのログイン操作を不要にし、受付の混雑を回避する狙い。
