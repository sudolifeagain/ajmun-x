/**
 * Discord Webhook Logger (Shared)
 *
 * Centralized logger implementation used by both app and bot.
 ...
 */
export async function log(level: LogLevel, title: string, context?: LogContext): Promise<void> {
    ...
    try {
        ...
        if (context?.details) {
            fields.push({
                name: "📋 詳細",
                value: context.details.slice(0, 1000), // Discord limit
                inline: false,
            });
        }
        ...
        // Silently fail - we don't want logging to break the main app
