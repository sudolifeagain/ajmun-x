...
import { resolvePrimaryGuild, toGuildMembershipInfo } from "./guildResolver";
...
export async function sendQRCodeDM(
    ...
    try {
        ...
        const membershipInfos = memberships.map(toGuildMembershipInfo);
        const { guildName } = await resolvePrimaryGuild(membershipInfos, attribute);
