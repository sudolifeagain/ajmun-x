 ...
 */
async function resolveStaffGuild(
 ...
 */
async function resolveOrganizerGuild(
    memberships: GuildMembershipInfo[]
): Promise<PrimaryGuildResult> {
    ...
    if (roleMapping) {
        const targetGuildIds = roleMapping.targetGuildIds
            .split(",")
            .map((id: string) => id.trim());
        const targetMembership = memberships.find((m) =>
            targetGuildIds.includes(m.guildId)
        );
        if (targetMembership) {
            return {
 ...
 */
function resolveParticipantGuild(
    memberships: GuildMembershipInfo[]
): PrimaryGuildResult {
    const targetGuild = memberships.find((m) => m.isTargetGuild);
    return {
 ...
 */
export function toGuildMembershipInfo(
    membership: {
        guildId: string;
        roleIds: string;
        guild: {
